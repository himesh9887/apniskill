...
export default function SkillCard({ user, onSwapRequest, ctaLabel = 'Request Swap' }) {
  ...
  return (
    <MotionArticle whileHover={{ y: -6 }} className="glass-card flex h-full flex-col gap-5 p-5 sm:gap-6 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-2">
          <p className="text-sm uppercase tracking-[0.24em] text-sky-300">Community Match</p>
          <h3 className="text-xl font-semibold text-white sm:text-2xl">{user.name}</h3>
          <p className="text-sm text-slate-400">{user.headline}</p>
        </div>
        <div className="rounded-2xl bg-white/5 px-3 py-2 text-left sm:text-right">
